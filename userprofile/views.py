...
def profile(request):
    if request.method == 'POST':
        # Retrieve source and destination values from the form
        s = request.POST.get('from')
        r = request.POST.get('to')

        # Split and process the source and destination values
        s2 = s
        r2 = r
        s1 = s.lower()
        s = s1.split(',', 1)
        ...
        # Print source and destination for testing
        print(r + " " + s)

        # Retrieve obstacles based on the source and destination
        ...
        # Retrieve the selected mode of travel
        mode = request.POST.get('mode')

        # Render index1.html with the necessary parameters
        return render(request, "index1.html", {'r': r2, 's': s2, 'obst': obst_locations, 'mode': mode})
    else:
